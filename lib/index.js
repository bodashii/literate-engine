...
Magic.prototype.showManagerEmployees = async function () {
  ...
  const prompt = await inquirer
    ...
    .then(async (response) => {
      ...
      const prompt = await db
        ...
        .query(
          `SELECT
                      employee, employee.first_name, employee.last_name, roles.title
                      AS position, departments.name
                      AS department, roles.salary
                      AS salary, CONCAT(manager.first_name, ' ', manager.last_name)
                      AS manager
                      FROM employees employee
                      INNER JOIN roles ON employee.role_id = roles.id
                      INNER JOIN departments ON roles.department_id = departments.id
                      RIGHT JOIN employees manager ON manager.id = employee.manager_id
                      WHERE manager.id = ${
                        managerIds[managers.indexOf(response.manager)]
                      }
                      GROUP BY manager.id`
        )
        .then(([rows, fields]) => {
...
Magic.prototype.viewDepartmentExpense = async function () {
  const departments = this.departments.map((department) => {
    return department.name;
  });
...
Magic.prototype.updateRole = async function () {
  const arr = this.employees.map((employee) => {
    return `${employee.first_name} ${employee.last_name}`;
  });

  const ids = this.employees.map((employee) => {
    return employee.id;
  });

  const position = this.roles.map((role) => {
    return role.title;
  });

  const positionIds = this.roles.map((role) => {
    return role.id;
  });
  ...
  const prompt = await inquirer
    ...
    .then((data) => {
      db.query(`UPDATE employees
                      SET role_id = ${positionIds[position.indexOf(data.role)]}
                      WHERE id = ${ids[arr.indexOf(data.employee)]}`);
      return data;
    })
    .then((data) => {
      this.seedDatabase();
      const note = `Updated employee role.`;
      this.mainMenu(note);
    });
};
...
Magic.prototype.mainMenu = async function () {
  ...
  const prompt = await inquirer
    ...
    .then((menu) => {
      ...
      } else if (menu.choice === "Show employees by Manager") {
        this.showManagerEmployees().then(() => {
          this.options("employee");
        });
      } else if (menu.choice === "View total utilized budget per department") {
        this.viewDepartmentExpense().then(() => {
          this.options("department");
        });
